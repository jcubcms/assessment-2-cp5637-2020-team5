...
if( ! function_exists( 'edunxt_site_branding' ) ) :
	/**
  	 * Site Branding
  	 *
  	 * @since 1.0.0
  	 */
function edunxt_site_branding() { ?>
    <div class="wrapper">
        ...
        <nav id="site-navigation" class="main-navigation" role="navigation" aria-label="Primary Menu">
            ...
            wp_nav_menu( array(
                ...
                'menu_class'     => 'nav-menu',
                'fallback_cb'    => 'edunxt_primary_navigation_fallback',
            ) );
            ?>
        </nav><!-- #site-navigation -->
    </div><!-- .wrapper -->
...
add_action( 'edunxt_action_header', 'edunxt_site_branding', 10 );
...
if ( ! function_exists( 'edunxt_footer_top_section' ) ) :
   ...
   * @since 1.0.0
   */
  function edunxt_footer_top_section() {
      $footer_sidebar_data = edunxt_footer_sidebar_class();
...
add_action( 'edunxt_action_footer', 'edunxt_footer_top_section', 10 );

if ( ! function_exists( 'edunxt_footer_section' ) ) :
  ...
  function edunxt_footer_section() { ?>
        <div class="site-info">
            <?php 
                ...
                // Powered by content.
                $powered_by_text = sprintf( __( ' Theme edunxt by %s', 'edunxt' ), '<a target="_blank" rel="designer" href="http://sensationaltheme.com/">Sensational Theme</a>' );
            ?>
            <div class="wrapper">
...
add_action( 'edunxt_action_footer', 'edunxt_footer_section', 20 );

if ( ! function_exists( 'edunxt_footer_sidebar_class' ) ) :
  /**
   * Count the number of footer sidebars to enable dynamic classes for the footer
   *
   * @since edunxt 0.1
   */
  function edunxt_footer_sidebar_class() {
    $data = array();
...
if ( ! function_exists( 'edunxt_excerpt_length' ) ) :
   ...
   */
  function edunxt_excerpt_length( $length ) {

    if ( is_admin() ) {
      return $length;
    }

    $excerpt_length = edunxt_get_option( 'excerpt_length' );
...
add_filter( 'excerpt_length', 'edunxt_excerpt_length', 999 );

if( ! function_exists( 'edunxt_banner_header
  ' ) ) :
    /**
     * Page Header
    */
    function edunxt_banner_header() { 
        ...
        elseif( is_front_page() ) {
            ...
            $header_image_url = edunxt_banner_image( $image_url = '' );
        } ?>
...
add_action( 'edunxt_banner_header', 'edunxt_banner_header', 10 );

if( ! function_exists( 'edunxt_banner_title' ) ) :
/**
 * Page Header
*/
...
if( ! function_exists( 'edunxt_banner_image' ) ) :
/**
 * Banner Header Image
*/
function edunxt_banner_image( $image_url ){
    global $post;

    $archive_header = edunxt_get_option( 'archive_header_image' );
...
if ( ! function_exists( 'edunxt_posts_tags' ) ) :
    /**
     * Prints HTML with meta information for the current post-date/time and author.
     */
    function edunxt_posts_tags() {
 ...
 * @since 1.0
 */
function edunxt_render_social_links() {
